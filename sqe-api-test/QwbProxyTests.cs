...
public partial class WebControllerTest
{
	...
	public async Task CanGetQwbVariants(bool realtime)
	{
		...
		Assert.False(string.IsNullOrEmpty(result.variants.First().bibliography.First().shortTitle));

		Assert.False(
				string.IsNullOrEmpty(result.variants.First().bibliography.First().pageReference));
