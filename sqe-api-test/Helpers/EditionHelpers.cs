...
*/
				...
				, uint?                   textFragmentId = null)
		{
			if (textFragmentId.HasValue)
			{
				...
				if (!textFragmentRequestObject.HttpResponseObject.textFragments.Any(
						x => x.lines.Any(
								y => y.signs.Count(
											 z => z.signInterpretations.Any(
													 a => a.attributes.Any(
															 b => b.attributeValueId == 1)))
									 > 2)))
				{
					throw new Exception(
			...
			foreach (var textFragment in textFragments)
			{
				...
				if (textFragmentRequestObject.HttpResponseObject.textFragments.Any(
						x => x.lines.Any(y => y.signs.Count > 2)))
					return textFragmentRequestObject.HttpResponseObject;
			}
