...
*/
	...
	public static uint GetEditionId()
	{
		_editionCount =
				(_editionCount + 1)
				% NumberOfUsableEditions; // Increment the counts (loop back at end)
			...
			, bool                    auth      = false)
	{
		...
		await getEditionObject.SendAsync(client, requestUser: user, auth: auth);
	...
	public static async Task<uint> CreateCopyOfEdition(
			HttpClient                client
			, uint                    editionId       = 0
			, string                  name            = ""
			, Request.UserAuthDetails userAuthDetails = null)
	{
		...
		if (string.IsNullOrEmpty(name))
			name = "test-name-" + _editionCount;

		var newScrollRequest = new Post.V1_Editions_EditionId(
				editionId
				, new EditionCopyDTO(name, null, null));
		...
		var (httpMsg, httpResp) = (newScrollRequest.HttpResponseMessage
								   , newScrollRequest.HttpResponseObject);
			...
			, Request.UserAuthDetails           userAuthDetails = null)
	{
		...
		var (response, msg) = (deleteResponse1.HttpResponseMessage
							   , deleteResponse1.HttpResponseObject);
		...
		if (shouldSucceed)
		{
			...
			var (response2, msg2) = (deleteResponse2.HttpResponseMessage
									 , deleteResponse2.HttpResponseObject);
	...
	public static async Task<List<TextFragmentDataDTO>> GetEditionTextFragments(
			uint                      editionId
			, HttpClient              client
			, Request.UserAuthDetails user = null)
	{
	...
	public static async Task<TextEditionDTO> GetEditionTextFragmentWithSigns(
			uint                      editionId
			, HttpClient              client
			, Request.UserAuthDetails user           = null
			, uint?                   textFragmentId = null)
	{
		if (textFragmentId.HasValue)
		{
			...
			if (!textFragmentRequestObject.HttpResponseObject.textFragments
										  .Any(x => x.lines.Any(y => y.signs.Count(z => z
																						.signInterpretations
																						.Any(a => a
																								  .attributes
																								  .Any(b => b
																	 ...
																	 > 2)))
			{
				throw new Exception(
						$"The edition {
							...
							textFragmentId.Value
						} doesn't have any lines with 3 or more signs");
			}
		...
		foreach (var textFragment in textFragments)
		{
			...
			if (textFragmentRequestObject.HttpResponseObject.textFragments
										 .Any(x => x.lines.Any(y => y.signs.Count > 2)))
				return textFragmentRequestObject.HttpResponseObject;
		}

		throw new Exception(
				$"The edition {editionId} doesn't have any fragments with 3 or more signs");
	}

	public static async Task<EditionListDTO> GetAllEditions(
			HttpClient                          client
	...
	public class EditionCreator : IDisposable
	{
		...
		{
			EditionId = await CreateCopyOfEdition(
					_client
					, EditionId
					, _name
					, _userAuthDetails);
