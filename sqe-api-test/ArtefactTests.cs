...
public partial class WebControllerTest
{
	...
	public async Task<ArtefactListDTO> GetEditionArtefacts(uint? editionId = null)
	{
		...
		var (response, artefactResponse, artefactRtResponse) = (
				artRequest.HttpResponseMessage, artRequest.HttpResponseObject
	...
	private static (decimal scale, decimal rotate, int translateX, int translateY, int zIndex)
			ArtefactPosition() => (1.1m, 45m, 34765, 556, 2);
	...
	public async Task CanBatchUnplaceArtefacts()
	{
					 ...
					 new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			var (updateResponse, updatedArtefacts) =
					await Request
							.SendHttpRequestAsync<BatchUpdateArtefactPlacementDTO,
									...
									{
											artefactPlacements = allArtefacts
																 .Select(x => new
	...
	public async Task CanCreateArtefacts()
	{
					 ...
					 new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			const string newArtefactShape =
					"POLYGON((0 0, 200 0, 200 200, 0 200, 0 0),(77 80, 77 92, 102 92, 102 80, 77 80),(5 5, 5 25, 25 25, 25 5, 5 5))";
	...
	public async Task CanGetSuggestedTextFragmentForArtefact()
	{
		...
		var path = $"/v1/editions/{
			editionId
		}/artefacts/{
			artefactId
		}/text-fragments?optional=suggested";

		// Act
		var (tfResponse, tfData) =
				await Request.SendHttpRequestAsync<string, ArtefactTextFragmentMatchListDTO>(
						_client
						, HttpMethod.Get
						, path
						, null);
	...
	public async Task CannotCreateArtefactsOnUnownedEdition()
	{
					 ...
					 new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			const string newArtefactShape =
					"POLYGON((0 0, 0 200, 200 200, 0 200, 0 0),(5 5, 25 5, 25 25, 5 25, 5 5),(77 80, 102 80, 102 92, 77 92, 77 80))";
	...
	public async Task CannotCreateMalformedArtefact()
	{
					 ...
					 new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			// Act
			var newArtefactObject =
					new Post.V1_Editions_EditionId_Artefacts(newEdition, newArtefact);

			await newArtefactObject.SendAsync(_client, auth: true, shouldSucceed: false);

			var (artefactResponse, _) = (
			...
			(artefactResponse, _) = (newArtefactObject.HttpResponseMessage
			...
			newArtefact = new CreateArtefactDTO
			{
					...
					, placement = new PlacementDTO
					{
							scale = 2.43567m
							, rotate = newRotate
							, // 0–9999.99 is the only allowable range
							translate = new TranslateDTO
							{
									x = newTranslateX
									, y = newTranslateY
	...
	public async Task CanUpdateArtefacts()
	{
					 ...
					 new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			// Arrange (update all)
			var (otherScale, otherRotate, otherTranslateX, otherTranslateY, otherzIdx) =
					ArtefactPosition();
			...
			Assert.True(
					_wkr.Read(artefact.mask)
						.EqualsTopologically(_wkr.Read(updatedAllArtefact.mask)));
	...
	public async Task CanGetEditionArtefactRois()
	{
					 ...
					 new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			// Act
			var getArtefactRois =
					new Get.V1_Editions_EditionId_Artefacts_ArtefactId_Rois(newEdition, artefactId);

			await getArtefactRois.SendAsync(_client, StartConnectionAsync, true);

			getArtefactRois.HttpResponseObject.ShouldDeepEqual(
					getArtefactRois.SignalrResponseObject);
