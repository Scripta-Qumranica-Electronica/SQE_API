...
namespace SQE.ApiTest
{
	...
	{
		private Request.UserAuthDetails UserUpdateRequestDTOToUserAuthDetails(
		...
		private async Task<UserObj> GetUserByEmail(string email, bool shouldSucceed = true)
		{
			...
			var users =
					await _db.RunQueryAsync<UserObj>(
							checkForUserSQL
							, checkForUserSQLParams); // Get user from DB
		...
		private async Task<DetailedUserTokenDTO> Login(
				...
				, bool            realtime      = false)
		{
			var request = new Post.V1_Users_Login(
					new LoginRequestDTO
					{
							email = user.email
							, password = user.password
		...
		/// <returns></returns>
		private async Task CreateActivatedUserAsync(
				NewUserRequestDTO user
				, bool            shouldSucceed = true)
		{
			var (_, newUser) =
					await CreateUserAccountAsync(
							user
							, shouldSucceed); // Asserts already in this function

			await ActivatUserAccountAsync(
					newUser
					, shouldSucceed); // Asserts already in this function
		}
		...
		/// <param name="shouldSucceed">Set to false if this request is supposed to fail</param>
		/// <returns></returns>
		private async Task<NewUserRequestDTO> CreateRandomActivatedUserAsync(
				bool shouldSucceed = true)
		{
			...
			await CreateActivatedUserAsync(
					user
					, shouldSucceed); // Asserts already in sub functions.
		...
		public async Task AccountDetailsUpdateFailsWithWrongPassword()
		{
			...
			using (var userCreator = new UserHelpers.UserCreator(user, _client, _db))
			{
				...
				Assert.Equal(
						...
						, updateUserAccountObject.HttpResponseMessage.StatusCode);
			}
		}
		...
		public async Task AccountPasswordUpdateFailsWithWrongEnteredPassword()
		{
			...
			using (var userCreator = new UserHelpers.UserCreator(user, _client, _db))
			{
				...
				Assert.Equal(
						HttpStatusCode.Unauthorized
						, passwordUpdateRequest.HttpResponseMessage.StatusCode);
			}
		}
		...
		public async Task CanRegisterUserAccount()
		{
			...
			var user = new NewUserRequestDTO(
					...
					, "🦊🦆"
					, "💩"
					, null); // Let's try some modern person with an emoji for a name to ensure unicode multibyte success.

			using (var userCreator = new UserHelpers.UserCreator(
					user
					, _client
					, _db
					, false))
			{
				...
				await ActivatUserAccountAsync(
						userDetails
						, realtime: true); // Asserts already in this function

				// Act (login)
				var userAuth = UserUpdateRequestDTOToUserAuthDetails(user);

				var userLoginRequest = new Post.V1_Users_Login(
						new LoginRequestDTO
						{
								email = user.email
								, password = user.password
				...
				await userLoginRequest.SendAsync(_client, auth: true, requestUser: userAuth);
		...
		public async Task CanResendAccountActivation()
		{
			...
			try
			{
				...
				Thread.Sleep(
						2000); // The time resolution of the database date_created field is 1 second.
				...
				Assert.True(
						newTokenCreationTime.date_created
						>= tokenCreationTime
								.date_created); // Make sure the token date the same or higher
			}
			finally
			{
				// Cleanup (remove user)
		...
		public async Task CanUpdateDetailsBeforeActivation()
		{
			...
			try
			{
				...
				// Update user details
				var updateUser = new NewUserRequestDTO(
						user.email
						, user.password
						, "ACME"
						, "Joe"
				...
				(response, updatedUser) =
						await CreateUserAccountAsync(
								updateUser); // Asserts already in this function
		...
		public async Task ChangeActivatedAccountPassword(bool realtime)
		{
			...
			// Act (change password)
			var request = new Post.V1_Users_ChangePassword(
					new ResetLoggedInUserPasswordRequestDTO
					{
			...
			await request.SendAsync(
					realtime
							...
							: _client
					, StartConnectionAsync
					, requestUser: new Request.UserAuthDetails
					{
							Email = user.email
							, Password = user.password
							,
		...
		public async Task ChangeUnactivatedAccountEmail(bool realtime)
		{
			...
			var request = new Post.V1_Users_ChangeUnactivatedEmail(
					new UnactivatedEmailUpdateRequestDTO
					{
							email = user.email
							, newEmail = newEmail
							,
		...
		public async Task ResetActivatedAccountPassword(bool realtime)
		{
			...
			var request = new Post.V1_Users_ForgotPassword(
			...
			var tokenRequest = new Post.V1_Users_ChangeForgottenPassword(
					new ResetForgottenUserPasswordRequestDTO
					{
		...
		public async Task CanGetUserStoreData()
		{
			...
			userDataRequest.HttpResponseObject.ShouldDeepEqual(
					userDataRequest.SignalrResponseObject);
		...
		private async Task WriteToUserDataStore(
				bool              realtime
				, string          dataString
				, bool            shouldSucceed
		...
		{
			// Arrange
			var dataObj = new UserDataStoreDTO { data = dataString };
