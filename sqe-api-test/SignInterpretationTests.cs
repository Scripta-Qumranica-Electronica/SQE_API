...
public partial class WebControllerTest
{
	...
	private async Task<SignInterpretationDTO> GetEditionSignInterpretation(uint editionId)
	{
		...
		foreach (var textRequest in
				 textFragments.textFragments.Select(tf => new Get.
															V1_Editions_EditionId_TextFragments_TextFragmentId(
																	editionId
																	, tf.id)))
		{
	...
	public async Task CanAddAttributeToEdition()
	{
			   ...
			   new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			Assert.Single(
					newAttrs.attributes.Where(x => (x.attributeName == newAttribute.attributeName)
												   && (x.description == newAttribute.description)));

			var returnedNewAttr =
					newAttrs.attributes.First(x => (x.attributeName == newAttribute.attributeName)
												   && (x.description == newAttribute.description));

			Assert.Single(returnedNewAttr.values);

			Assert.Single(
					returnedNewAttr.values.Where(x => (x.value == newAttrValues.First().value)
													  && (x.description
														  == newAttrValues.First().description)
													  && (x.cssDirectives
	...
	public async Task CanCreateNewAttributeForSignInterpretation()
	{
			   ...
			   new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			var responseAttr =
					httpData.attributes.FirstOrDefault(x => x.attributeValueId
															== signInterpretationAddAttribute
																	.attributeValueId);
	...
	public async Task CanCreateVariantSignInterpretation()
	{
			   ...
			   new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			var (_, line, previousSignInterpretation, copySignInterpretation, nextSignInterpretation
					) = await _getTestingSignInterpretationData(editionId);
			...
			var request = new Post.V1_Editions_EditionId_SignInterpretations_SignInterpretationId(
					editionId
					, copySignInterpretation
					, newSignInterpretation);
			...
			Assert.Contains(
					...
					, x => x.nextSignInterpretationId
						   == originalSignInterpretation.HttpResponseObject.signInterpretationId);
	...
	public async Task CanLinkSignInterpretations()
	{
			   ...
			   new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			var signStream =
					(await EditionHelpers.GetEditionTextFragmentWithSigns(
							...
							, Request.DefaultUsers.User1)).textFragments
														  .Where(x => x.lines.Any(y => y.signs.Count
																					   > 2))
			...
			Assert.Empty(
					firstSignInterpretation.nextSignInterpretations
										   .Where(x => x.nextSignInterpretationId
													   == lastSignInterpretation
	...
	public async Task CanUnlinkSignInterpretations()
	{
			   ...
			   new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			var nextSignInterpretation =
					signStream.First(x => firstSignInterpretation.nextSignInterpretations
																 .Any(y => y
...
* point one"
	...
	public async Task CanGetAttributesOfSpecificSignInterpretation()
	{
			   ...
			   new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			var request = new Get.V1_Editions_EditionId_SignInterpretations_SignInterpretationId(
					editionId
					, signInterpretation.signInterpretationId);
	...
	private async
			Task<(TextFragmentDTO Fragments, LineDTO Line, uint PreviousSignInterpretationId, uint
					NextSignInterpretationId, uint AfterNextSignInterpretation)>
			...
			, bool                        realtime = false)
	{
		...
		Assert.NotEqual(
				0u
				, newSignInterpretationRequest.CreatedSignInterpretation.signInterpretations.First()
											  .signId);
		...
		Assert.Contains(
				signs
				, x => x.signInterpretations.Any(y => y.IsDeepEqual(
														 responseObject.created.First())));
		...
		var interpretationMatchingCreate = signs
										   .FirstOrDefault(x => x.signInterpretations
																 .Any(y => y.signInterpretationId
																		   == newlyCreatedInterpretation
																				   .signInterpretationId))
										   ?.signInterpretations
										   .First(y => y.signInterpretationId
													   == newlyCreatedInterpretation
															   .signInterpretationId);
		...
		{
			var attrMatch =
					interpretationMatchingCreate.attributes.First(x => x.attributeValueId
			...
			, bool            realtime  = false)
	{
		...
		Assert.Empty(
				signs.Where(x => x.signInterpretations.Any(y => y.signInterpretationId
																== newlyCreatedInterpretationId)));
		...
		Assert.NotEmpty(
				flattenedSigns.Where(x => flattenedSigns.First()
														.nextSignInterpretations
														.Any(y => y.nextSignInterpretationId
