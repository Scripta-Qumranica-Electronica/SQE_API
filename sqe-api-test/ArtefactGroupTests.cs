...
public partial class WebControllerTest
{
	...
	/// <returns></returns>
	private async
			Task<(HttpResponseMessage httpResponseMessage, ArtefactGroupListDTO httpResponseBody,
					ArtefactGroupListDTO signalrResponse)> _getArtefactGroupsAsync(
					uint                      editionId
	...
	/// <returns></returns>
	private async
			Task<(HttpResponseMessage httpResponseMessage, ArtefactGroupDTO httpResponseBody,
					ArtefactGroupDTO signalrResponse)> _getArtefactGroupAsync(
					uint                      editionId
					...
					, Request.UserAuthDetails user          = null)
	{
		...
		// Act
		var getApiRequest =
				new Get.V1_Editions_EditionId_ArtefactGroups_ArtefactGroupId(
						editionId
						, artefactGroupId);
	...
	private async
			Task<(HttpResponseMessage httpResponseMessage, ArtefactGroupDTO httpResponseBody,
					ArtefactGroupDTO signalrResponse, ArtefactGroupDTO listenerResponse)>
			_createArtefactGroupAsync(
					uint                      editionId
					, string                  artefactGroupName
					, List<uint>              artefacts
					, bool                    shouldSucceed = true
					, Request.UserAuthDetails user          = null
					, Request.UserAuthDetails user2         = null
					, bool                    realtime      = false)
	{
		// Arrange
		user ??= Request.DefaultUsers.User1;
		...
		var createApiRequest =
				new Post.V1_Editions_EditionId_ArtefactGroups(editionId, artefactGroup);
		...
		var (httpMessage, httpBody, signalr, listener) = (
				createApiRequest.HttpResponseMessage, createApiRequest.HttpResponseObject
				, createApiRequest.SignalrResponseObject, createApiRequest.CreatedArtefactGroup);
					...
					, bool                    realtime      = false)
	{
		...
		// Act
		var updateApiRequest = new Put.V1_Editions_EditionId_ArtefactGroups_ArtefactGroupId(
				editionId
				, artefactGroupId
				, artefactGroup);
		...
		var (httpMessage, httpBody, signalr, listener) = (
				updateApiRequest.HttpResponseMessage, updateApiRequest.HttpResponseObject
				, updateApiRequest.SignalrResponseObject, updateApiRequest.UpdatedArtefactGroup);
	...
	private async
			Task<(HttpResponseMessage httpResponseMessage, DeleteIntIdDTO httpResponseBody,
					DeleteIntIdDTO
					signalrResponse, DeleteIntIdDTO listenerResponse)> _deleteArtefactGroupAsync(
					uint                      editionId
					, uint                    artefactGroupId
					, bool                    shouldSucceed = true
					, Request.UserAuthDetails user          = null
					...
					, bool                    realtime      = false)
	{
		...
		var deleteApiRequest =
				new Delete.V1_Editions_EditionId_ArtefactGroups_ArtefactGroupId(
						editionId
						, artefactGroupId);
		...
		var (httpMessage, httpBody, signalr, listener) = (
				deleteApiRequest.HttpResponseMessage, deleteApiRequest.HttpResponseObject
		...
		if (shouldSucceed)
		{
			...
			Assert.Equal(
					EditionEntities.artefactGroup
					, deleteApiRequest.DeletedArtefactGroup.entity);
	...
	public async Task CanCreateAndDeleteArtefactGroups(bool realtime)
	{
			   ...
			   new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			var artefacts = (await ArtefactHelpers.GetEditionArtefacts(editionId, _client))
					.artefacts;
			...
			// Act
			var (_, createdArtefactGroupHttp, createdArtefactGroupRt, _) =
					await _createArtefactGroupAsync(
							editionId
							, artefactGroupName
							, new List<uint>
							{
			...
			var createdArtefactGroup = realtime
					? createdArtefactGroupRt
					: createdArtefactGroupHttp;
	...
	public async Task CannotPerformBadUpdateToArtefactGroup(bool realtime)
	{
			   ...
			   new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			var badArtefactId = (uint)Enumerable.Range(1, 100000)
												.FirstOrDefault(num => !validArtefacIds.Contains(
																		(uint)num));
			...
			// Act
			var (updateHttpMsg, _, _, _) = await _updateArtefactGroupAsync(
					editionId
					, createdArtefactGroup.id
					, updatedArtefactGroupName
					, updatedArtefacts
					, false
					, realtime: realtime);
			...
			var (_, artefactGroup, _) = await _getArtefactGroupAsync(
					editionId
					, createdArtefactGroup.id);
	...
	public async Task CannotReuseArtefactsInArtefactGroup(bool realtime)
	{
			   ...
			   new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			// Act
			var (secondHttpMsg, _, _, _) = await _createArtefactGroupAsync(
					editionId
					, secondArtefactGroupName
					, secondGroupArtefacts
					, false
					, realtime: realtime);
			...
			Assert.Equal(
					createdArtefactGroup.artefacts
	...
	public async Task CanUpdateArtefactGroups(bool realtime)
	{
			   ...
			   new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			while (artefacts.Count < 5) // Make sure we have enough artefacts for the test
			{
				...
				artefacts = (await ArtefactHelpers.GetEditionArtefacts(editionId, _client))
						.artefacts;
			}
			...
			// Act
			var (_, updatedAgHttp, updatedAgRt, _) = await _updateArtefactGroupAsync(
					editionId
					, createdArtefactGroup.id
					, updatedArtefactGroupName
					, updatedArtefacts
