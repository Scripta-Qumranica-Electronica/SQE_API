...
public partial class WebControllerTest
{
	...
	private static void _verifyLineTextDTO(LineTextDTO msg)
	{
		...
		Assert.NotNull(
				msg.signs.First().signInterpretations.First().attributes.First().attributeString);
	...
	public async Task CanAddTextFragmentAfter()
	{
					 ...
					 new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			Assert.Empty(
					textFragments.textFragments.Where(x => (x.id == msg.id)
														   || (x.name == msg.name)));
	...
	public async Task CanAddTextFragmentToEnd()
	{
					 ...
					 new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			// Act
			var (_, msg) = await _createTextFragment(editionId, textFragmentName, realtime: true);
	...
	public async Task CanDeleteLine(bool realtime)
	{
					 ...
					 new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			// Act
			var deleteLine = new Delete.V1_Editions_EditionId_Lines_LineId(editionId, lastLineId);
	...
	public async Task CanUpdateTextChunk(bool realtime, string replacementString)
	{
					 ...
					 new EditionHelpers.EditionCreator(_client, StartConnectionAsync))
		{
			...
			Assert.Equal(replacementString, updatedString.Substring(0, replacementString.Length));
		}
	}
