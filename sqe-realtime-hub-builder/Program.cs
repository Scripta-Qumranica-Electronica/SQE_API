...
";
	...
	private static void VerifyControllerClassName(IEnumerable<MemberDeclarationSyntax> members)
	{
		...
		{
			throw new Exception(
					$"Each Controller file should have only one class, this file has {
						classes.Count
					} classes.");
		}
		...
		foreach (var attr in controllerAttrs)
		{
			...
			if (attr.Attributes.Count(x => (x.Name.ToString()
											 .IndexOf(
													 "Route"
			...
			, IEnumerable<MemberDeclarationSyntax> members)
	{
		...
		using (var outputFile =
			   new StreamWriter(Path.Combine(hubFolder, $"{controllerName}Hub.cs")))
		{
			// Write disclaimer to file
			outputFile.Write(_autogenFileDisclaimer);

			// Collect and write using statements
			foreach (var element in root.Usings.Where(element => element.Name.ToString()
			...
			, string                controllerName)
	{
		...
		if (attributes.Count(x => (x.Name.ToString()
		...
		{
			throw new Exception(
					$"The SQE API only supports endpoints with a single route path attribute. The method {
		...
		foreach (var attribute in attributes)
		{
			...
			else
			{
				...
				foreach (var argument in attribute.ArgumentList.Arguments)
				{
					...
					{
						throw new Exception(
								$"The SQE API only allows paths using token replacement with the [controller] and [action] tokens. The method {
									method.Identifier.Text
								} violates this policy.");
					}
				}
	...
	{
		var subscriptionHub = new StreamReader(Path.Combine(projectRoot, "SubscriptionHub.cs.txt"))
				.ReadToEnd();
