...
namespace SQE.SqeHttpApi.DataAccess
{
    ...
    public class TextRepository : DbConnectionBase, ITextRepository
    {
        ...
        private async Task<TextEdition> _getEntityById(UserInfo user, uint startId, uint endId)
        {
            ...
            using (var connection = OpenConnection())
            {
                var scrolls = await connection.QueryAsync<TextEdition>(
                    ...
                    types: new []{typeof(TextEdition), typeof(TextFragment), typeof(Line), typeof(Sign), 
        ...
        private static ushort _getNewTextFragmentPosition(uint? previousFragmentId, uint? nextFragmentId,
            List<TextFragmentData> textFragmentIds)
        {
            ...
            if (!previousFragmentId.HasValue && !nextFragmentId.HasValue)
            ...
            if (previousTextFragment.Count() != 1) // The specified previous text fragment does not exist in the edition
                throw new StandardErrors.ImproperInputData("textFragmentId");
            ushort previousPosition = previousTextFragment.First().Position;
            
            // If there is also a nextPosition, verify that previousPosition and nextPosition are sequential
            if (nextPosition.HasValue && previousPosition + 1 != nextPosition) // The specified previous and next text fragments are not sequential
                throw new StandardErrors.ImproperInputData("textFragmentId"); 
        ...
        private async Task<uint> _createTextFragmentDataAsync(UserInfo user, uint textFragmentId, string textFragmentName)
        {
        ...
        private async Task<uint> _createTextFragmentPosition(UserInfo user, uint textFragmentId,
            ushort position)
        {
            // Set the parameters for the mutation object
            ...
            // Create the mutation object
            var createTextFragmentPositionMutation = new MutationRequest(MutateType.Create, fragmentPositionParameters,
                "text_fragment_sequence");
                
            // Commit the mutation
            var textFragmentMutationResults =
                await _databaseWriter.WriteToDatabaseAsync(user, new List<MutationRequest>() {createTextFragmentPositionMutation});
                
            // Ensure that the entry was created
            if (textFragmentMutationResults.Count != 1 || !textFragmentMutationResults.First().NewId.HasValue)
        ...
        private async Task _shiftTextFragmentsPosition(UserInfo user, IReadOnlyCollection<TextFragmentData> textFragmentList,
            ushort startPosition, int offset)
        {
            // Create the mutation objects
            var textFragmentShiftMutations = textFragmentList.Where(x => x.Position >= startPosition)
                .Select(x =>
                {
                    if (x.Position + offset < 0 || x.Position + offset > 65535)
                        throw new StandardErrors.DataNotWritten("change textFragment position", "the desired position is out of range"); 
