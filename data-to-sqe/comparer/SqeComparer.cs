...
{
	public static Dictionary<string, string> NormalizedSigns = new()
	{
			{ "", "W" }
			, { "־", "X" }
			, { "°", "X" }
	...
	public static List<ChangeIds> Compare(Line sqeLine, Line sourceLine)
	{
		...
		_printList(sourceLine, sqeLine, bestResult);
	...
	private static int _calculatePenalty(Line sourceLine, Line sqeLine, List<ChangeIds> changeIds)
	{
		var penalty = 0;

		foreach (var changeId in changeIds)
		{
			if (changeId.SqeId == null)
				penalty += 10;
			else if (changeId.SourceId == null)
				penalty += 5;
			else
			{
				var sourceSign = sourceLine.GetSignInterpretationById(changeId.SourceId.Value)
										   .Character;
					...
					&& data.SourceId != null)
				{
					...
					if (sourceSign.Equals(sqeSign))
					{
						...
						runs = true;
						 ...
						 && data.SqeId != null)
				{
					var sourceSign = sourceLine.GetSignInterpretationById(nextData.SourceId.Value)
											   .Character;
		...
		foreach (var diffBlock in diffResult.DiffBlocks)
		{
				   ...
				   && sqeListPosition < sqeSequence.NumberOfInterpretations())
			{
				_changeIds.Add(
						...
						{
								SourceId =
										sourceSequence.GetSignInterpretationIdAtPosition(
												sourceListPosition++)
								, SqeId = sqeSequence.GetSignInterpretationIdAtPosition(
										sqeListPosition++)
								,
						});
			}
