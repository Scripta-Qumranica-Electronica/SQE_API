...
public class Line
{
	...
	private readonly List<SignInterpretation> _signInterpretations = new();
	...
	public Line(Scroll textEdition, LineData lineData)
	{
		...
		if (LineData.Signs.Last().SignInterpretations[0].NextSignInterpretations.Count > 0)
		{
			EndAnchorId = LineData.Signs.Last()
								  ...
								  .NextSignInterpretations[0].NextSignInterpretationId;
		}

		;

		LineData.Signs.RemoveAll(s => s.SignInterpretations
									   .Any(si => si.Attributes.Any(a => a.AttributeValueId == 9)));
	...
	public List<SignInterpretationSequence> getSequences()
	{
		...
		foreach (var signInterpretation in _signInterpretations.FindAll(s => s.SignId
																			 == LineData.Signs[0]
																						.SignId))
			...
			, uint sourceSignInterpretationId)
	{
		...
		{
			var sourceSignInterpretation =
					sourceSignData.SignInterpretations.Find(s => s.Character.Equals(
																	sqeSignInterpretation
																			.Character));
	...
	{
		var signData =
				LineData.Signs.Find(s => s.SignInterpretations.Any(si => si.SignInterpretationId
	...
	{
		return
				LineData.Signs.Find(s => s.SignInterpretations.Any(si
																		   => si.Attributes
																				.Any(a => a
																								  .AttributeValueId
																						  == (uint)(
