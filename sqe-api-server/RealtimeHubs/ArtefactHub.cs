...
public async Task<ArtefactDTO> PostV1EditionsEditionIdArtefacts(uint editionId, CreateArtefactDTO payload)

    {
        try
        {
             return  await _artefactService.CreateArtefactAsync(await _userService.GetCurrentUserObjectAsync(editionId, true), payload);
        }
...
public async Task DeleteV1EditionsEditionIdArtefactsArtefactId(uint editionId, uint artefactId)

    {
        try
        {
              await _artefactService.DeleteArtefactAsync(await _userService.GetCurrentUserObjectAsync(editionId, true), artefactId);
        }
        catch (ApiException err)
        {
            throw new HubException(JsonSerializer.Serialize(new HttpExceptionMiddleware.ApiExceptionError(nameof(err), err.Error, err is IExceptionWithData exceptionWithData ? exceptionWithData.CustomReturnedData : null)));
...
public async Task<BatchUpdatedArtefactTransformDTO> PostV1EditionsEditionIdArtefactsBatchTransformation(uint editionId, BatchUpdateArtefactPlacementDTO payload)

    {
        try
        {
             return  await _artefactService.BatchUpdateArtefactTransformAsync(await _userService.GetCurrentUserObjectAsync(editionId, true), payload);
        }
        catch (ApiException err)
        {
            throw new HubException(JsonSerializer.Serialize(new HttpExceptionMiddleware.ApiExceptionError(nameof(err), err.Error, err is IExceptionWithData exceptionWithData ? exceptionWithData.CustomReturnedData : null)));
...
/// <summary>
	...
	/// <returns></returns>
[Authorize]
public async Task<DeleteIntIdDTO> DeleteV1EditionsEditionIdArtefactGroupsArtefactGroupId(uint editionId, uint artefactGroupId)
...
[Authorize]
public async Task<DiffReconstructedResponseDTO> PutV1EditionsEditionIdArtefactsArtefactIdDiffReplaceTranscription(uint editionId, uint artefactId, DiffReplaceReconstructionRequestDTO payload)

    {
        try
        {
             return  await _textService.DiffReplaceReconstructedText(await _userService.GetCurrentUserObjectAsync(editionId, true), artefactId, payload);
        }
        catch (ApiException err)
        {
            throw new HubException(JsonSerializer.Serialize(new HttpExceptionMiddleware.ApiExceptionError(nameof(err), err.Error, err is IExceptionWithData exceptionWithData ? exceptionWithData.CustomReturnedData : null)));
