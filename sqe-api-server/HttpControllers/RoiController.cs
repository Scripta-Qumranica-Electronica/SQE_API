...
public class RoiController : ControllerBase
{
	...
	public async Task<ActionResult<InterpretationRoiDTO>> GetRoi(
			...
			, [FromRoute] uint roiId) => await _roiService.GetRoiAsync(
	...
	public async Task<ActionResult<InterpretationRoiDTO>> CreateRoi(
			[FromRoute]  uint                    editionId
			, [FromBody] SetInterpretationRoiDTO newRoi) => await _roiService.CreateRoiAsync(
	...
	public async Task<ActionResult<InterpretationRoiDTOList>> CreateRois(
			[FromRoute]  uint                        editionId
			, [FromBody] SetInterpretationRoiDTOList newRois) => await _roiService.CreateRoisAsync(
			await _userService.GetCurrentUserObjectAsync(editionId, true)
			, newRois);
	...
	public async Task<ActionResult<BatchEditRoiResponseDTO>> BatchEditRois(
			[FromRoute]  uint            editionId
			, [FromBody] BatchEditRoiDTO rois) => await _roiService.BatchEditRoisAsync(
			await _userService.GetCurrentUserObjectAsync(editionId, true)
			, rois);
	...
	public async Task<ActionResult<UpdatedInterpretationRoiDTO>> UpdateRoi(
			...
			, [FromBody]  SetInterpretationRoiDTO updateRoi) => await _roiService.UpdateRoiAsync(
			await _userService.GetCurrentUserObjectAsync(editionId, true)
			, roiId
			, updateRoi);
	...
	public async Task<ActionResult> DeleteRoi([FromRoute] uint editionId, [FromRoute] uint roiId)
