...
public class ArtefactController : ControllerBase
{
	...
	public async Task<ActionResult<ArtefactDTO>> CreateArtefact(
			...
			, [FromBody] CreateArtefactDTO payload) => await _artefactService.CreateArtefactAsync(
	...
	public async Task<ActionResult<ArtefactDTO>> GetArtefact(
			...
			, [FromQuery] List<string> optional) => await _artefactService.GetEditionArtefactAsync(
	...
	public async Task<ActionResult<ArtefactGroupDTO>> GetEditionArtefactGroup(
			[FromRoute]   uint editionId
			, [FromRoute] uint artefactGroupId) => await _artefactService.GetArtefactGroupDataAsync(
			await _userService.GetCurrentUserObjectAsync(editionId)
			, artefactGroupId);
	...
	public async Task<ActionResult<DeleteIntIdDTO>> DeleteEditionArtefactGroup(
			[FromRoute]   uint editionId
			, [FromRoute] uint artefactGroupId) => await _artefactService.DeleteArtefactGroupAsync(
			await _userService.GetCurrentUserObjectAsync(editionId, true)
			, artefactGroupId);
	...
	public async Task<ActionResult<DiffReconstructedResponseDTO>> UpdateReconstructedTranscription(
			[FromRoute]   uint                                editionId
			, [FromRoute] uint                                artefactId
			, [FromBody]  DiffReplaceReconstructionRequestDTO payload)
