...
{
	public static CatalogueMatchDTO ToDTO(this CatalogueMatch cat) => new()
	{
			...
			, matchConfirmationAuthor = cat.MatchConfirmationAuthor
			, dateOfConfirmation = cat.MatchConfirmationDate
			, matchId = cat.MatchId
			, name = cat.Name
			, proxy = cat.Proxy
			, suffix = cat.Suffix
	...
	{
		return new CatalogueMatchListDTO { matches = catList.Select(x => x.ToDTO()).ToArray() };
	}
