...
public static partial class ExtensionsDTO
{
				...
				, characters = sac.Characters.Select(x => new SignInterpretationDTO
								  {
										  ...
										  , attributes = x.Attributes.Select(y => y.ToDTO())
														  .ToArray()
										  , nextSignInterpretations =
												  x.NextCharacters.Select(z => z.ToDTO()).ToArray()
										  , rois = x.Rois.Select(a => new InterpretationRoiDTO
													{
															...
															shape = MergeSignInterpretationRois(
																	x.Rois)
	...
	private static string MergeSignInterpretationRois(IEnumerable<SpatialRoi> rois)
	{
		...
		var positionedRois = rois.Select(h =>
										 {
	...
	/// <param name="sac"></param>
	/// <returns></returns>
	public static PlacementDTO PlacementDTO(this ScriptArtefactCharacters sac) => new()
	{
	...
	/// <param name="ca"></param>
	/// <returns></returns>
	public static InterpretationAttributeDTO ToDTO(this CharacterAttribute ca) => new()
	{
	...
	public static NextSignInterpretationDTO ToDTO(this CharacterStreamPosition csp)
		=> new() { nextSignInterpretationId = csp.NextSignInterpretationId };

	public static List<GlyphDataDTO> ToDTO(this IEnumerable<Glyph> glyphs)
		=> glyphs.Select(x => x.ToDTO()).ToList();

	public static GlyphDataDTO ToDTO(this Glyph glyph) => new()
	{
	...
	public static KernPairDTO ToDTO(this KerningPair kern) => new()
	{
