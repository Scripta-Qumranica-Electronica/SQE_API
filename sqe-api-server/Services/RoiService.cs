...
public class RoiService : IRoiService
{
	...
	private readonly Regex                            _removeDecimals = new(@"\.\d+");
	private readonly IRoiRepository                   _roiRepository;
	private readonly WKTReader                        _wkr = new();
	private readonly WKTWriter                        _wkw = new();
	...
	private async Task<UpdatedInterpretationRoiDTOList> UpdateRoisInternalAsync(
			UserInfo                                  editionUser
			, IEnumerable<UpdateInterpretationRoiDTO> updatedRois) => new()
	{
			rois = (await _roiRepository.UpdateRoisAsync( // Write new rois
							editionUser
							, updatedRois.ToSignInterpretationRoiData().ToList())).ToUpdateDTO()
																				  .ToList()
			,
	};
