...
namespace SQE.API.Server.Services
{
	...
	public class ImagedObjectService : IImagedObjectService
	{
		...
		public async Task<SimpleImageListDTO> GetImagedObjectImagesAsync(string imagedObjectId)
			=> (await _repo.GetImagedObjectImagesAsync(HttpUtility.UrlDecode(imagedObjectId)))
					.ToDTO();
				...
				, List<string> optional       = null)
		{
			...
			var result = new ImagedObjectListDTO { imagedObjects = new List<ImagedObjectDTO>() };
				...
				, bool   withMasks = false)
		{
			...
			result.imagedObjects = result.imagedObjects.GroupJoin(
												 artefacts.artefacts
												 , // Take two lists: result.imagedObjects and artefacts.artefacts.
												 arg => arg.id
												 , // Group them together where the imagedObject.id
												 arg => arg.imagedObjectId
												 , // == the artefactObject.imagedObjectId (a one-to-many relationship).
												 (imagedObject, artefactObjects)
														 => new
																 ImagedObjectDTO // For each such grouping create an object that merges
																 {
																		 id = imagedObject.id
																		 , // the data from the imagedObject
																		 ...
																		 , artefacts =
																				 artefactObjects
																						 .ToList()
																		 , // and the group of artefactObjects as a List.
																 })
		...
		public async Task<ImagedObjectDTO> GetImagedObjectAsync(
				UserInfo       editionUser
				, string       imagedObjectId
				, List<string> optional = null)
		{
			...
			if (artefacts)
			{
				var artefactList = ArtefactListSerializationDTO.QueryArtefactListToArtefactListDTO(
						(await _artefactRepository.GetEditionArtefactListAsync(editionUser, masks))
						.ToList()
						, editionUser.EditionId.Value);
		...
		private static void ParseOptionals(
