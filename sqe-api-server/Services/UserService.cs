...
namespace SQE.API.Server.Services
{
	...
	{
		Task<DetailedUserTokenDTO> AuthenticateAsync(
				string   email
				, string password
		...
		Task<UserInfo> GetCurrentUserObjectAsync(
				uint?  editionId
				, bool write   = false
				, bool locking = false
				, bool admin   = false);

		Task<DetailedUserTokenDTO> CreateNewUserAsync(
				NewUserRequestDTO newUserData
				, string          clientId = null);

		Task<DetailedUserTokenDTO> UpdateUserAsync(
				uint?                  userId
				, UserUpdateRequestDTO updateUserData
				, string               clientId = null);

		Task<NoContentResult> UpdateUnactivatedAccountEmailAsync(
				string   oldEmail
				, string newEmail
				, string clientId = null);
		...
		Task<NoContentResult> ChangePasswordAsync(
				uint?    userId
				, string oldPassword
				, string newPassword
				, string clientId = null);
		...
		Task<NoContentResult> ResetLostPasswordAsync(
				string   token
				, string password
				, string clientId = null);
		...
		Task<NoContentResult> SetUserDataStoreAsync(
				uint?              userId
				, UserDataStoreDTO data
				, string           clientId = null);
	}
	...
	public class UserService : IUserService
	{
		...
		/// </returns>
		public async Task<DetailedUserTokenDTO> AuthenticateAsync(
				string   email
				, string password
				, string clientId = null)
		{
			...
			return new DetailedUserTokenDTO
			{
					...
					, token = result.Activated
							? BuildUserToken(result.Email, result.UserId)
							: null
		...
		public async Task<DetailedUserTokenDTO> CreateNewUserAsync(
				NewUserRequestDTO newUserData
				, string          clientId = null)
		{
		...
		/// <param name="clientId"></param>
		/// <returns>Returns a UserDTO for the newly created user account.</returns>
		public async Task<DetailedUserTokenDTO> UpdateUserAsync(
				uint?                  userId
				, UserUpdateRequestDTO updateUserData
				, string               clientId = null)
		{
			...
			var originalUserInfo =
					await _userRepository.GetDetailedUserByIdAsync(GetCurrentUserId());

			var resetActivation = (updateUserData.email != null)
								  && (originalUserInfo.Email != updateUserData.email);
			...
			{
				updatedUserWithInfo =
						await _userRepository.CreateUserActivateTokenAsync(updateUserData.email);
		...
		/// <param name="clientId"></param>
		/// <returns></returns>
		public async Task<NoContentResult> ConfirmUserRegistrationAsync(
				string   token
				, string clientId = null)
		{
...
The Scripta Qumranica Electronica team</body></html>";
			...
			const string emailSubject =
					"Confirmation of your Scripta Qumranica Electronica account activation";

			var name = !string.IsNullOrEmpty(userInfo.Forename)
					   || !string.IsNullOrEmpty(userInfo.Surname)
			...
			await _emailSender.SendEmailAsync(
					userInfo.Email
					, emailSubject
					, emailBody.Replace("$User", name).Replace("$WebServer", webServer));
		...
		/// <param name="clientId"></param>
		/// <returns></returns>
		public async Task<NoContentResult> UpdateUnactivatedAccountEmailAsync(
				string   oldEmail
				, string newEmail
				, string clientId = null)
		{
		...
		/// <param name="clientId"></param>
		/// <returns></returns>
		public async Task<NoContentResult> ResendActivationEmail(
				string   email
				, string clientId = null)
		{
			try
		...
		/// <param name="clientId"></param>
		/// <returns></returns>
		public async Task<NoContentResult> RequestResetLostPasswordAsync(
				string   email
				, string clientId = null)
		{
			try
...
The Scripta Qumranica Electronica team</body></html>";
				...
				const string emailSubject =
						"Lost password for your Scripta Qumranica Electronica account";
		...
		/// <param name="clientId"></param>
		/// <returns></returns>
		public async Task<NoContentResult> ChangePasswordAsync(
				uint?    userId
				, string oldPassword
				, string newPassword
				, string clientId = null)
		...
		/// <param name="clientId"></param>
		/// <returns></returns>
		public async Task<NoContentResult> ResetLostPasswordAsync(
				string   token
				, string password
				, string clientId = null)
		{
...
The Scripta Qumranica Electronica team</body></html>";
			...
			const string emailSubject =
					"Reset password for your Scripta Qumranica Electronica account";
		...
		/// <returns></returns>
		public async Task<NoContentResult> SetUserDataStoreAsync(
				uint?              userId
				, UserDataStoreDTO data
				, string           clientId = null)
		{
		...
		/// <returns></returns>
		public async Task<UserInfo> GetCurrentUserObjectAsync(
				uint?  editionId
				, bool write   = false
				, bool locking = false
				, bool admin   = false)
		{
			var access = new EditionAccessLevels(
					true
					, write
					, locking
					, admin);
...
The Scripta Qumranica Electronica team</body></html>";
			...
			var name = !string.IsNullOrEmpty(userWithInfo.Forename)
					   || !string.IsNullOrEmpty(userWithInfo.Surname)
