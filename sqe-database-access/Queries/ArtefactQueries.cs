...
{
	private const string _artefactIdRestriction = " AND artefact_data.artefact_id = @ArtefactId";
...
// Add a subselect to check the ordering of the artefacts based on the order of
// text fragments and lines if possible, then they can be ordered as is already done.
// Get the order with a subselect.
...
FROM artefact_data_owner
    JOIN edition USING(edition_id)
	JOIN edition_editor USING(edition_id)
	JOIN artefact_data USING(artefact_data_id)

    LEFT JOIN (
	    SELECT artefact_id, region_in_sqe_image, sqe_image_id, edition_editor_id
		FROM artefact_shape
		JOIN artefact_shape_owner ON artefact_shape_owner.edition_id = @EditionId
		    AND artefact_shape_owner.artefact_shape_id =  artefact_shape.artefact_shape_id) ash USING(artefact_id)
...
$Order";
	...
	private const string _mask = "ASTEXT(ash.region_in_sqe_image)";
