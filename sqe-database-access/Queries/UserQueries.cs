...
WHERE $Where";
				...
				, bool         whereAnd = true)
		{
			...
			return _query.Replace( // Add the columns to the query
								 ...
								 , string.Join(
										 ","
										 , columns.Select(x => $"{x} AS {x.ToPascalCase()}")))
...
ON DUPLICATE KEY UPDATE `type` = @Type, date_created = NOW()";
		...
		public static string GetQuery(bool emailOnly = false) => _query.Replace(
				"$VALUES"
				, emailOnly
...
";
		...
		public static string GetQuery(bool resetActivation) => _query.Replace(
				"$Activated"
				, resetActivation
						? ", activated = 0"
						: "");
