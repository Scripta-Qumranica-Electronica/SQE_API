...
namespace SQE.DatabaseAccess.Models
{
	public class SignInterpretationCommentaryData
	{
		...
		public uint?  SignInterpretationCommentaryCreatorId { get; set; }
		public uint?  SignInterpretationCommentaryEditorId  { get; set; }
		public uint?  SignInterpretationId                  { get; set; }
		public uint?  AttributeId                           { get; set; }
		public string Commentary                            { get; set; }
	}

	public class SignInterpretationCommentaryDataSearchData : SignInterpretationCommentaryData
															  , ISearchData
	{
		...
		public string getSearchParameterString()
		{
			...
			if (SignInterpretationCommentaryId.HasValue)
			{
				searchParameters.Add(
						$"sign_interpretation_commentary_id = {SignInterpretationCommentaryId.Value}");
			}

			if (AttributeId.HasValue)
				searchParameters.Add($"attribute_id = {AttributeId.Value}");

			if (!string.IsNullOrEmpty(Commentary))
				searchParameters.Add($"commentary like '{Commentary}'");

			if (!string.IsNullOrEmpty(CommentaryRegex))
				searchParameters.Add($"commentary regexp '{CommentaryRegex}'");
