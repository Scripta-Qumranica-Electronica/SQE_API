...
namespace SQE.DatabaseAccess
{
	...
	public static class DatabaseCommunicationRetryPolicy
	{
		...
		private static readonly List<uint> _retrySqlExceptions = new List<uint>
		{
				1205
				, 1213
				, 1412
		...
		public static async Task ExecuteRetry(
				Func<Task>          operation
	...
	public class DatabaseCommunicationCircuitBreakPolicy
	{
		...
		public DatabaseCommunicationCircuitBreakPolicy()
		{
			...
			_circuitBreakPolicy = Policy
								  .Handle<MySqlException>(
										  exception => _pauseExceptions.Contains(exception.Code))
								  .CircuitBreaker(
										  RetryCount
										  , TimeSpan.FromSeconds(CircuitBreakerPause));
		}
	...
	public class ReliableMySqlConnection : DbConnection
	{
		...
		public ReliableMySqlConnection(
				...
				, DatabaseCommunicationCircuitBreakPolicy circuitBreakPolicy) : this(
				circuitBreakPolicy) => ConnectionString = connectionString;
	...
	public class ReliableMySqlDbCommand : DbCommand
	{
		...
		public ReliableMySqlDbCommand(
				...
				, DbConnection  connection
				, DbTransaction transaction) : this(command, connection)
		...
		protected override DbParameter CreateDbParameter()
			=> _underlyingSqlCommand.CreateParameter();
		...
		{
			return DatabaseCommunicationRetryPolicy.ExecuteRetry(
					() => _underlyingSqlCommand.ExecuteNonQuery());
		}
		...
		{
			return DatabaseCommunicationRetryPolicy.ExecuteRetry(
					() => _underlyingSqlCommand.ExecuteScalar());
		}
