...
public static class DatabaseCommunicationRetryPolicy
{
	...
	private static readonly List<uint> _retrySqlExceptions = new()
	{
			1205
			, 1213
			, 1412
	...
	private static readonly AsyncPolicy _retryPolicyAsync = Policy
															...
															.WaitAndRetryAsync(
																	...
																	, attempt
																			  => TimeSpan
																					  .FromMilliseconds(
																							  WaitBetweenRetriesInMilliseconds /*_waitTime(attempt)*/)
																	, (
																			  exception
																			  , delay
																			  , retryCount
	...
	public static async Task<TResult> ExecuteRetry<TResult>(
...
public class DatabaseCommunicationCircuitBreakPolicy
{
	...
	private static readonly List<uint>  _pauseExceptions = new() { 1040, 1203 };
	...
	public DatabaseCommunicationCircuitBreakPolicy()
	{
		_circuitBreakerRetryPolicyAsync = Policy
										  .Handle<
												  MySqlException>(exception => _pauseExceptions
		...
		_circuitBreakerRetryPolicy = Policy
									 .Handle<
											 MySqlException>(exception => _pauseExceptions.Contains(
																	 exception.Code))
									 .WaitAndRetry(
											 RetryCount
											 , attempt => TimeSpan.FromMilliseconds(
		...
		_circuitBreakPolicyAsync = Policy
								   .Handle<MySqlException>(exception => _pauseExceptions.Contains(
																   exception.Code))
								   .CircuitBreakerAsync(
										   RetryCount
	...
	public async Task ExecuteRetryWithCircuitBreaker(
			Func<Task>          operation
			, CancellationToken cancellationToken)
	{
	...
	public async Task<TResult> ExecuteRetryWithCircuitBreaker<TResult>(
			Func<Task<TResult>> operation
			, CancellationToken cancellationToken) => await _circuitBreakerRetryPolicyAsync
															.WrapAsync(_circuitBreakPolicyAsync)
															.ExecuteAsync(operation.Invoke);
}
...
public class ReliableMySqlConnection : DbConnection
{
	...
	public ReliableMySqlConnection(
	...
	protected override DbCommand CreateDbCommand()
		=> new ReliableMySqlDbCommand(_underlyingConnection.CreateCommand());
...
public class ReliableMySqlDbCommand : DbCommand
{
	...
	public ReliableMySqlDbCommand(MySqlCommand command) => _underlyingSqlCommand = command;

	public ReliableMySqlDbCommand(MySqlCommand command, DbConnection connection) : this(command)
		=> DbConnection = connection;

	public ReliableMySqlDbCommand(
			MySqlCommand    command
			, DbConnection  connection
			, DbTransaction transaction) : this(command, connection) => DbTransaction = transaction;
	...
	protected override DbParameterCollection DbParameterCollection
		=> _underlyingSqlCommand.Parameters;
	...
	{
		return DatabaseCommunicationRetryPolicy.ExecuteRetry(() => _underlyingSqlCommand
