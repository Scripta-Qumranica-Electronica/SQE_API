...
public static class TableData
{
	...
	private static readonly List<SingleTableData> Data = new()
	{
			new SingleTableData
			{
					Name = "manuscript"
					, HasData = true
					, Terminators = new uint[] { 14, 15 }
					,
			}
			, new SingleTableData
			{
	...
	public static string Parent(Table table) => table > 0
			? Data[(int)table - 1].Name
			: "";
	...
	/// <param name="table"></param>
	/// <returns>Name of parent database table</returns>
	public static string Child(Table table) => table < LastTable
			? Data[(int)table + 1].Name
			: "";
	...
	public static string ConnectingTableToParent(Table table) => table > 0
			? $"{Name(table)}_to_{Parent(table)}"
			: null;
	...
	public static string ConnectingTableToChild(Table table) => table < LastTable
			? $"{Name(table)}_to_{Child(table)}"
			: "";
	...
	public static string DataTableName(Table table) => HasData(table)
			? $"{Name(table)}_data"
			: null;
	...
	public static uint[] AllTerminators(Table table, TerminatorType terminatorType)
	{
		...
		for (var i = table; i <= (LastTable - 1); i++)
			terminators[i - table] = Terminators(i)[(int)terminatorType];
