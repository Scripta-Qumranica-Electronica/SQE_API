...
public class SignStreamMaterializationRepository(IDatabaseAccessor dba) : ISignStreamMaterializationRepository
{
			...
			, IReadOnlyDictionary<uint, BasicSignInterpretation> signDict)
	{
		...
		// that failed for whatever reason.
		{
			...
			while ((waitCount <= 50)
				   && existingRequests.Any(x => (x.EditionId == editionId)
			...
			if (existingRequests.Any(x => (x.EditionId == editionId)
		...
		return paths.Select(x =>
							{
								...
								foreach (var signInterpretationId in x)
								{
									...
									{
										if (signInterpretation.Attributes
															  .Any(x => x.AttributeValueId == 2))
											text.Append(" ");
					...
					.QueryAsync<BasicSingleSignInterpretation, BasicSignInterpretationAttribute,
							...
							, (sign, attribute) =>

							  {
								  ...
								  signDict.Add(sign.SignInterpretationId, expandedInterpretation);
