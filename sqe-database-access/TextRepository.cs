		...
		, ISignStreamMaterializationRepository    _materializationRepository) : ITextRepository
{
			...
			, uint     anchorAfter  = 0)
	{
...

		{
			...
			lineData.Signs.Add(
					CreateTerminatorSign(TableData.Table.line, TableData.TerminatorType.End));
...
WHERE text_fragment_to_line.line_id = @LineId AND text_fragment_to_line_owner.edition_id = @EditionId";
	...
	public async Task<LineData> UpdateLineAsync(UserInfo editionUser, uint lineId, string lineName)
	{
	...
	public async Task UpdateSignInterpretationDataAsync(
			...
			, SignInterpretationData signInterpretationData)
	{
		var signInterpretaionId = signInterpretationData.SignInterpretationId.GetValueOrDefault();
	...
	public async Task<List<uint>> GetAllSignInterpretationIdsForSignIdAsync(
			UserInfo editionUser
			, uint   signId) => (await dba.QueryAsync<uint>(
...

		{
			...
			foreach (var newSign in newSigns)
			{
				...
				if (signInterpretationId.HasValue)
				{
					...
					if (newSign.SignInterpretations.Any(x => (x.Attributes == null)
															 || (x.Commentaries == null)
						...
						|| !anchorsBefore.Any())
					{
						...
						newSign.SignInterpretations = newSign.SignInterpretations.Select(x =>
																						 {
																							 x.Attributes ??=
						...
						if (!anchorsAfter.Any())
						{
							anchorsAfter = originalSignInterpretationData.NextSignInterpretations
																		 .Select(x => x
				...
				anchorsBefore.AddRange(
						newSignData.SignInterpretations.Select(si => si.SignInterpretationId
																	   .GetValueOrDefault()));
			}
			...
			, bool              materializeSignStream = true)
	{
		...
		foreach (var (secondSignInterpretation, index) in
				 secondSignInterpretationsList.Select((x, idx) => (x, idx)))
		{
		...
		if (secondSignInterpretationsData.Any(x => x.Attributes.Any(y => y.AttributeValueId.HasValue
																		 && _signAttributeControlCharacters
																				 .Contains(
																						 y.AttributeValueId
																						  .Value))))
		{
	...
	{
		{
			foreach (var signInterpretation in signInterpretations)
			{
				...
				*/
				...
				signInterpretation.SignInterpretationRois.ForEach(roi => roi.SignInterpretationId =
																		  signInterpretation
																				  .SignInterpretationId);
			...
			, uint?            nextFragmentId)
	{
		...
		{
			// Create the new text fragment abstract id
			var newTextFragmentId = await _simpleInsertAsync(TableData.Table.text_fragment);
	...
	public async Task<List<ArtefactDataModel>> GetArtefactsAsync(
			UserInfo editionUser
			, uint   textFragmentId) => (await dba.QueryAsync<ArtefactDataModel>(
	...
	public async Task<IEnumerable<CachedTextEdition>> GetCachedTextEdition(
			UserInfo editionUser
			, uint   textFragmentId) => await dba.QueryAsync<CachedTextEdition>(
	...
	public async Task<List<TextFragmentData>> GetFragmentDataAsync(UserInfo editionUser)
		=> (await dba.QueryAsync<TextFragmentData>(
					...
					, ArtefactModel                      artefact = null)
	{
...

		{
			...
			if (artefact != null)
			{
				...
				{
					throw new StandardExceptions.InputDataRuleViolationException(
							$"The submitted artefact must be associated by one and only one text fragment, the submitted artefact is associated with {
								textFragmentIds.Count()
							} text fragments.");
				}
			...
			var groupedPrimaryStream = primaryStream.GroupBy(
					p => p.signInterpretationId
					, p => p.AttributeValueId
					, (key, g) => new
					{
							signInterpretationId = key
							, AttributeValueIds = g.ToList()
							,
					});

			foreach (var entry in groupedPrimaryStream)
			{
				if (charactersWithValue.Any(x => entry.AttributeValueIds.Any(y => y == x)))
					break;

				lastLinkedSign = entry.signInterpretationId;

				primaryStream = primaryStream
								.Where(x => x.signInterpretationId != entry.signInterpretationId)
								.ToList();
			}
			...
			foreach (var entry in groupedPrimaryStream.Reverse())
			{
				if (charactersWithValue.Any(x => entry.AttributeValueIds.Any(y => y == x)))
					break;
			...
			if (string.IsNullOrEmpty(replacementText))
			{
				...
				var deleteSignInterpretationIds = primaryStream
												  .Where(
														  (x => x.signInterpretationId
			...
			var diff = differ.CreateCharacterDiffs(
			...
			var replacementTextList = replacementText.ToList().Select(x => x.ToString()).ToList();
			...
			foreach (var block in diff.DiffBlocks)
			{
				var anchorSignInterpretationId = block.DeleteStartA > 0
						? originalStringIdKey[block.DeleteStartA - 1]
						: priorSignInterpretationId;

				deleteSignIntIds.AddRange(
						originalStringIdKey.GetRange(block.DeleteStartA, block.DeleteCountA));

				var transferLength = block.DeleteCountA > block.InsertCountB
						? block.InsertCountB
						: block.DeleteCountA;

				for (var i = 0; i < transferLength; i++)
				{
					var replaceSiId = deleteSignIntIds[deleteSignIntIds.Count - transferLength + i];

					alterSignIntIds.Add(replaceSiId, replacementTextList[block.InsertStartB + i]);

					if (reconstructedRois.TryGetValue((uint)(block.InsertStartB + i), out var roi))
					{
						alteredRois.Add((replaceSiId, roi.Shape, roi.TranslateX, roi.TranslateY));
				...
				if (block.InsertCountB - transferLength > 0)
				{
					addSignIntIds.Add(
							...
							, Enumerable
							  ...
							  .Select<int, (string, (string shape, int x, int y)?)>(x =>
																					{
																						if
																								(reconstructedRois
																								 .TryGetValue(
			...
			foreach (var alterSignIntId in alterSignIntIds)
			{
				var spaceCharacter = alterSignIntId.Value == " ";
			...
			var remainingOriginalSignInterpretations = originalStringIdKey
													   ...
													   .Where(x => !deletedSignInterpretations
																		   .Contains(x.val)
																   && !alterSignIntIds.ContainsKey(
																		   x.val)
																   && originalString[x.idx] != ' ')
													   .ToList();

			foreach (var (unchangedSignWithRoi, idx) in
					 unchangedSignsWithRois.Select((item, idx) => (item, idx)))
			{
			...
			foreach (var addSignIntId in addSignIntIds)
			{
				var lastSiId = addSignIntId.Key;
				...
				foreach (var character in addSignIntId.Value)
				{
					...
					var (newSigns, alteredSigns) = await CreateSignWithSignInterpretationAsync(
							...
							, currentLineId
							, newSign
							, new List<uint> { lastSiId }
							, new List<uint>()
							, null
							, true
							, false);

					var newSignInterpretationIds = newSigns.SelectMany(x => x.SignInterpretations)
														   .Where(x => x.SignInterpretationId
																		.HasValue)
														   .Select(x => x.SignInterpretationId
																		 .Value);
			...
			if (createdRois.Any())
			{
				await _roiRepository.CreateRoisAsync(
						user
						, createdRois.Select(x => new SignInterpretationRoiData
									 {
											 SignInterpretationId =
			...
			if (alteredRois.Any())
			{
				var (currentCreatedRois, currentDeletedRois) =
						await _roiRepository.ReplaceSignInterpretationRoisAsync(
								user
								, alteredRois.Select(x => new SignInterpretationRoiData
											 {
													 SignInterpretationId =
															 x.signInterpretionId
													 , Shape = x.shape
			...
			var roiReplacements = replaceRoisResults
								  .Select(x => new UpdateEntity(
												  ...
												  , x.SignInterpretationRoiId.Value))
								  .ToList();
					...
					, objects =>
					  {
						  ...
						  if ((sectionId.HasValue)
							  && lastSignInterpretation.SignStreamSectionIds.All(x => x
																					  != sectionId
																							  .Value))
							  lastSignInterpretation.SignStreamSectionIds.Add(sectionId.Value);

						  if ((qwbWordId.HasValue)
							  && lastSignInterpretation.QwbWordIds.All(x => x != qwbWordId.Value))
	...
	private async Task<List<uint>> _getChildrenIds(
			UserInfo          user
			, TableData.Table table
			, uint            elementId) => (await dba.QueryAsync<uint>(
	...
	private async Task<uint> _getElementDataId(UserInfo user, TableData.Table table, uint elementId)
		=> await dba.QuerySingleAsync<uint>(
	...
	*/
			...
			, byte       priority                      = 0)
	{
		...
		signInterpretationCharacterParameters.Add("@sign_interpretation_id", signInterpretationId);
	...
	{
		{
			...
			// Commit the mutation
			var textFragmentMutationResults = await dba.WriteToDatabaseAsync(editionUser, requests);
	...
	private async
			Task<(PositionDataRequestHelper positionDataRequestFactory, uint? previousTextFragmentId
					, uint?
					nextTextFragmentId)> _createTextFragmentPositionRequestFactory(
					UserInfo     editionUser
					, uint?      anchorBefore
					, List<uint> textFragmentIds
					, uint?      anchorAfter)
