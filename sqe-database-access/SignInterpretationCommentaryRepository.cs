...
public class SignInterpretationCommentaryRepository(
		IDatabaseAccessor      dba
		, IAttributeRepository _attributeRepository) : ISignInterpretationCommentaryRepository
{
	/// <summary>
	...
	public async Task<List<SignInterpretationCommentaryData>> CreateCommentariesAsync(
			...
			, uint signInterpretationId
			, List<SignInterpretationCommentaryData> newCommentaries
			, IDbConnection connection = null) => await _createOrUpdateCommentariesAsync(
			editionUser
			, signInterpretationId
			, newCommentaries
			, MutateType.Create);
		...
		if (string.IsNullOrEmpty(commentary))
		{
			var signInterpretationCommentaryId = existingCommentaries
												 .Where(x => (x.AttributeId == attributeId)
															 && x.SignInterpretationCommentaryId
																 .HasValue)
			...
			, IDbConnection connection           = null)
	{
		...
		var requests = deleteCommentaryIds.Select(id =>
												  {
			...
			, uint   signInterpretationCommentaryId)
	{
		...
		var result = (await GetSignInterpretationCommentariesByDataAsync(editionUser, searchData))
				.ToList();
			...
			, IDbConnection                          connection = null)
	{
		...
		await DeleteAllCommentariesForSignInterpretationAsync(editionUser, signInterpretationId);
