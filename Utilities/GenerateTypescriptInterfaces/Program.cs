	...
	private const string _hubClassTemplate = @"namespace SQE.API.Server.RealtimeHubs
{
	...
	private const string _autogenFileDisclaimer = @"/* tslint:disable */
...
";
	...
	private static readonly Regex _rxl = new(
			@"List?<(?<return>.*)?>"
			, RegexOptions.Compiled | RegexOptions.IgnoreCase);

	private static readonly Regex _rxi = new(
			@"interface (?<type>.*?) "
			, RegexOptions.Compiled | RegexOptions.IgnoreCase);

	private static readonly Regex _rxp = new(
			@"\*\n\t \* @param.*\n\t \*"
			, RegexOptions.Compiled | RegexOptions.IgnoreCase);
	...
	private static void ParseSqeSignalrHub(
			string   projectRoot
			, string apiServerRoot
			, string tsFolder
			, string hubFolder)
	{
		Console.WriteLine(
				"Parsing the SignalR hub and creating corresponding typescript interfaces.");
		...
		apiServerRoot ??= Path.GetFullPath(
				Path.Combine(
						projectRoot
						, "../"
						, "../"
		...
		foreach (var method in members.OfType<MethodDeclarationSyntax>().ToList())
		{
			...
			var methodParams = string.Join(
					...
					, method.ParameterList.Parameters.Select(x => $"{
						...
						ConvertToTypescriptType(x.Type.ToString())
					}"));
	...
	private static string ParseComments(string comment)
	{
		...
		var parameterText = string.Join(
				...
				, parameters.Cast<XmlNode>()
							.Select(x => $" @param {x.Attributes["name"].Value} - {x.InnerText}"));
		...
		return $"/**{
			...
			(!string.IsNullOrEmpty(returnText)
					? " @returns - " + returnText
					: "")
		}\n/".Replace("\n", "\n\t *");
	}
	...
	private static string ConvertToTypescriptType(string type)
	{
			...
			&& type.Contains("Task"))
		{
			...
			type = matches.Count >= 1
					? ConvertToTypescriptType(matches[0].Groups["return"].Value)
					: "void";
		}
	...
	private static void WriteTsHub(
			List<MethodDesc>   hubMethods
			...
			outputFile.Write(
					template.Replace(
							...
							.Replace(
									...
									, string.Join(
											"\n"
											, hubInterfaceMethods.Select(x => _clientMethodTemplate
																			  .Replace(
																					  "$ONCOMMENT"
																					  , string
																							  .IsNullOrEmpty(
							...
							.Replace(
									...
									, string.Join(
											"\n"
											, hubMethods.Select(x => _serverMethodTemplate
	...
	private class Options
	{
		...
		[Option(
				...
				, "api-folder"
				, Required = false
				, HelpText = "Specify api server folder path.")]
		...
		[Option(
				...
				, "hub-folder"
				, Required = false
				, HelpText = "Specify signalr hub folder path.")]
