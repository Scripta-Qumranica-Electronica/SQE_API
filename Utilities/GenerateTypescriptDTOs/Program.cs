...
";
		...
		private static void ParseSqeDtoProject(
				string   projectRoot
				, string dtoFolder
				, string tsFolder)
		{
			Console.WriteLine(
					"Parsing the SQE DTOs and creating corresponding typescript interfaces.");
				...
				foreach (var classDescription in classes)
				{
					var intf = Interface.Replace("$Class", classDescription.typescriptType)
										.Replace(
												...
												, string.Join(
														"\n    "
														, classDescription.fields.Select(
																x => $"{x.name}{(x.nullable ? "?" : "")}: {x.typescriptType};")));
		...
		private static IEnumerable<EnumDescription> ParseSqeDtoEnums(FileSystemInfo file)
		{
			...
			foreach (var enumDeclaration in declaredEnums)
			{
				enums.Add(
						new EnumDescription(
								enumDeclaration.Identifier.ToString().ToPascalCase()
								, enumDeclaration.Members
												 .Select(x => x.Identifier.ToString().ToCamelCase())
												 .ToList()));
			}
		...
		private static IEnumerable<ClassDescription> ParseSqeDtoClasses(FileSystemInfo file)
		{
			...
			var compilation =
					CSharpCompilation.Create("SQEDTO", new[] { tree }, new[] { Mscorlib });
			...
			foreach (var dtoClass in members)
			{
				...
				foreach (var field in dtoClass.DescendantNodes()
											  .OfType<PropertyDeclarationSyntax>())
				{
					var (type, nullable) = ConvertToTypescriptType(
							field.Type
							, field.AttributeLists);

					var fieldDescription = new FieldDescription(
							field.Identifier.ToString().ToCamelCase()
							, type
							, nullable);
		...
		private static string ParseComments(string comment)
		{
			...
			var parameterText = string.Join(
					"\n"
					, parameters.Cast<XmlNode>()
								.Select(
										x => $" @param {x.Attributes["name"].Value} - {x.InnerText}"));
		...
		private static (string typeName, bool nullable) ConvertToTypescriptType(
				TypeSyntax                        type
				, SyntaxList<AttributeListSyntax> attrs = default)
		{
			...
			if (type is GenericNameSyntax genType)
			{
				switch (genType.Identifier.ToString())
				{
					...
					case "IEnumerable":
					{
						...
						return ($"Array<{ConvertToTypescriptType(args.FirstOrDefault()).typeName}>"
								, !required);
					}
			...
			{
				return (
						$"Array<{SimpleTypeToTypescript(type.ToString().Replace("[]", "")).typeName}>"
						, !required);
			}
		...
		private class Options
		{
			[Option(
					...
					, "project-folder"
					, Required = false
					, HelpText = "Specify project folder path.")]
			...
			[Option(
					...
					, "dto-folder"
					, Required = false
					, HelpText = "Specify dto folder path.")]
			...
			[Option(
					...
					, "ts-folder"
					, Required = false
					, HelpText = "Specify typescript definitions folder path.")]
