...
";
	...
	private static IEnumerable<ClassDescription> ParseSqeDtoClasses(FileSystemInfo file)
	{
		...
		var compilation = CSharpCompilation.Create("SQEDTO", new[] { tree }, new[] { Mscorlib });
			...
			, SyntaxList<AttributeListSyntax> attrs = default)
	{
		...
		if (type is GenericNameSyntax genType)
		{
			switch (genType.Identifier.ToString())
			{
				...
				case "Dictionary":
				{
					...
					return (
							$"{{ [key: {
								SimpleTypeToTypescript(args[0].ToString()).typeName
							}] : {
								SimpleTypeToTypescript(args[1].ToString()).typeName
							} }}", !required);
				}
			}
		}
		...
		{
			return ($"Array<{SimpleTypeToTypescript(type.ToString().Replace("[]", "")).typeName}>"
					, !required);
		}
