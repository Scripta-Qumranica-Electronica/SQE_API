...
";
	...
	public static async Task WriteEndpointsAsync(
			ApiRequestsDescription parsedControllerMethods
			, StreamWriter         outputFile)
	{
		foreach (var httpVerb in parsedControllerMethods.requests.OrderBy(x => x.Key.ToString())
														.Select(x => x.Key))
		{
        ...
        ");
			...
			if (!parsedControllerMethods.requests.TryGetValue(httpVerb, out var endpoints))
				continue;

			foreach (var endpoint in endpoints)
			{
				var endpointInputType = endpoint.IType == null
						? "EmptyInput"
						: endpoint.IType;

				var endpointOutputType = endpoint.OType == null
						? "EmptyOutput"
						: endpoint.OType;
            ...
            {{
                {
					string.Join(
							"\n"
							, endpoint.listeners.Select(x => $"public ListenerMethods {
								x.ParamName
							} = ListenerMethods.{
								x.ParamName
							};"))
            ...
            public Listeners AvailableListeners {{ get; }}";
						...
						: $@"AvailableListeners = new Listeners();
                {
					string.Join(
							"\n"
							, endpoint.listeners.Select(x => $"_listenerDict.Add(ListenerMethods.{
								x.ParamName
							}, ({
								x.ParamName
							}IsNull, {
				...
				var listenerMethods = endpoint.listeners.Count == 0
						...
						: string.Join(
								"\n"
								, endpoint.listeners.Select(x => $@"public {
									x.ParamType
								} {
									x.ParamName
								} {{ get; private set; }}
                ...
                {{
                    {
						string.Join(
								"\n"
								, constructorParams.Select(x => $"private readonly {
									x.ParamType
								} _{
									x.ParamName.ToCamelCase()
								};"))
					}
                    ...
                    public {
						...
						string.Join(
								...
								, constructorParams.Select(x => $"{
									...
									(endpoint.queryParams.Contains(x)
											? " = null"
											: "")
								}"))
					})
                    ...
                    {{
                        return RequestPath{
							string.Join(
									""
									, endpoint.routeParams.Select(x => $".Replace(\"/{
										x.ParamName.ToKebabCase()
                    ...
                    {
						(endpoint.routeParams.Any(x => x.ParamName.ToLowerInvariant()
													   == "editionid")
	...
	public static async Task WriteListenerEnumsAsync(
			List<ParameterDescription> listeners
			, StreamWriter             outputFile)
	{
