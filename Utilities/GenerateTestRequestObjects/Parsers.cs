...
namespace GenerateTestRequestObjects
{
    ...
    {
        public static (bool, string, string, string) GetMethodNameAndAuthorization(MethodDeclarationSyntax method,
        ...
        public static async Task ParseAndWriteClassesAsync(string testFolder, ClassDeclarationSyntax node,
            IEnumerable<SemanticModel> semModels, IEnumerable<MethodDescription<MethodDeclarationSyntax>> serviceMethods,
            IEnumerable<MethodDeclarationSyntax> broadcastMethods, Project project)
        {
            ...
            Console.WriteLine($"Writing to {Path.Combine(testFolder, "ApiRequests", $"{controllerName.ToPascalCase()}Requests.cs")}");
...
");
                ...
                {
                    var methodDescription = await ParseMethodsAsync(node,
                        semModels, serviceMethods, broadcastMethods, project, method, controllerName);

                    if (!parsedControllerMethods.requests.ContainsKey(methodDescription.RequestType))
                        parsedControllerMethods.requests.Add(methodDescription.RequestType, new List<ApiRequestEndpointDescription>());

                    if (parsedControllerMethods.requests.TryGetValue(methodDescription.RequestType, out var req))
        ...
        private static async Task<ApiRequestEndpointDescription> ParseMethodsAsync(ClassDeclarationSyntax node,
            IEnumerable<SemanticModel> semModels, IEnumerable<MethodDescription<MethodDeclarationSyntax>> serviceMethods, IEnumerable<MethodDeclarationSyntax> broadcastMethods,
            Project project, MethodDeclarationSyntax method, string controllerName)
        {
            ...
            methodDescription.OType = oType == "ActionResult" ? "EmptyOutput" : oType;

            // Format the method name and grab authorization
            var (anonymousAllowed, methodName, httpRequestType, httpPath) =
                Parsers.GetMethodNameAndAuthorization(method, controllerName);
            ...
            methodDescription.RequestType = httpRequestType.ToLowerInvariant() switch
            {
                ...
                _ => throw new Exception($"Unsupported http verb: {httpRequestType}")
            };
            ...
            foreach (var semModel in semModels)
            {
                try
                {
                    ...
                    var invokedSyntax = serviceMethods.FirstOrDefault(x =>
                    ...
                    if (!string.IsNullOrEmpty(methodDescription.listenerName))
                    {
                        ...
                        methodDescription.ListenerType = broadcastMethodDescription.ParameterList.Parameters.First().Type.ToString();
                    }
                }
                catch
                {
...
called in method {method.ToString()} of class {node.ToString()}");
        ...
        /// <param name="semModels"></param>
        /// <returns></returns>
        public static async Task<IEnumerable<MethodDescription<TSyntax>>> GetAllSyntaxNodesAsync<TSyntax>(
            Compilation compilation, IEnumerable<SemanticModel> semModels, List<string> namespaces)
        where TSyntax : SyntaxNode
        {
            return (await
                    compilation.SyntaxTrees.SelectManyAsync(async x =>
                        {
                            var results = (await x.GetRootAsync()).DescendantNodes().OfType<TSyntax>()
                                .Where(x =>
                                {
                                    NamespaceDeclarationSyntax namespaceDeclarationSyntax = null;
                                    if (!Helpers.SyntaxNodeHelper.TryGetParentSyntax(x, out namespaceDeclarationSyntax))
                                        return false;
                                    return namespaces.Contains(namespaceDeclarationSyntax.Name.ToString());
                                });
                            return results;
                        }
                    )
                ...
                .Select(x =>
                {
                    ...
                    return new MethodDescription<TSyntax>(default(string), x);
                });
        }
    }
}