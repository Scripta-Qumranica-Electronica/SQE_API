			...
			, string                                                  controllerName)
	{
		...
		foreach (var semModel in semModels)
		{
			try
			{
				...
				var invokedSyntax =
						serviceMethods.FirstOrDefault(x => x.Symbol == invokedSymbol.ToString());
				...
				while (match.Success)
				{
					...
					{
						var broadcastMethodDescription =
								broadcastMethods.First(x => x.Identifier.ToString()
															== listenerName);
						...
						if (!methodDescription.listeners.Any(x => (x.ParamName
																   == listenDescription.ParamName)
...
called in method {
	...
	{
		return (await compilation.SyntaxTrees.SelectManyAsync(async x =>
															  {
																  var results =
																		  (await x.GetRootAsync())
																		  .DescendantNodes()
																		  .OfType<TSyntax>()
																		  .Where(x =>
																				 {
