...
internal static class Program
{
	...
	private static async Task ParseSqeHttpControllers()
	{
		...
		// Collect all the methods in the services namespace
		var broadcastMethods = (await Parsers.GetAllSyntaxNodesAsync<InterfaceDeclarationSyntax>(
									   compilation
									   , semModels
		...
		foreach (var tree in compilation.SyntaxTrees)
		{
			...
			foreach (var node in rootSyntaxNode.DescendantNodes().OfType<ClassDeclarationSyntax>())
			{
				...
				if (namespaceDeclarationSyntax.Name.ToString() != "SQE.API.Server.HttpControllers")
					continue;
