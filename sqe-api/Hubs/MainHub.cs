...
namespace SQE.SqeApi.Server.Hubs
{
	public class MainHub : Hub
	{
		...
		private readonly IImagedObjectService __magedObjectService;
		private readonly IImageService __mageService;
		private readonly ITextService _textService;

		public MainHub(IArtefactService artefactService, IUserService userService, IEditionService editionService, IImagedObjectService magedObjectService, IImageService mageService, ITextService textService)
		{
			_artefactService = artefactService;
			_userService = userService;
			_editionService = editionService;
			__magedObjectService = magedObjectService;
			__mageService = mageService;
      ...
      createFragment);
		...
		/// <returns>An array of the text fregment ids in correct sequence</returns>		[AllowAnonymous]
		...
		/// <returns>An array of the line ids in the proper sequence</returns>		[AllowAnonymous]
      ...
      textFragmentId);
		...
		/// <returns>A manuscript edition object including the fragments and their lines in a hierarchical order and in correct sequence</returns>		[Authorize]
      ...
      textFragmentId);
		...
		/// <returns>A manuscript edition object including the fragments and their lines in a hierarchical order and in correct sequence</returns>		[AllowAnonymous]
      ...
      lineId);
		...
		/// <returns>A DetailedUserTokenDTO with a JWT for activated user accounts, or the email address of an unactivated user account</returns>		[AllowAnonymous]
		...
		{
			return await _userService.UpdateUnactivatedAccountEmailAsync(
      payload.email, 
      payload.newEmail);
		}
		...
		{
			return await _userService.ResetLostPasswordAsync(
      payload.token, 
      payload.password);
		}
      ...
      payload.newPassword);
		...
		/// <returns>Returns a DetailedUserDTO with the updated user account details</returns>		[Authorize]
      ...
      payload);
		...
		/// <returns>Returns a DetailedUserDTO for the confirmed account</returns>		[AllowAnonymous]
		...
		/// <returns>A DetailedUserDTO for user account.</returns>		[Authorize]
		...
		/// <returns>Returns a UserDTO for the newly created account</returns>		[AllowAnonymous]
