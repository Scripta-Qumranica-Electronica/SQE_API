...
namespace SQE.SqeApi.Server.Hubs
{
	public class MainHub : Hub
	{
		...
		public MainHub(IEditionService editionService, IUserService userService)
		{
		...
		{
			return await _editionService.ChangeEditionEditorRights(_userService.GetCurrentUserObject(editionId),
				payload);
		}
		...
		{
			return await _editionService.DeleteEditionAsync(_userService.GetCurrentUserObject(editionId), token,
				optional);
		}
		...
		{
			return await _editionService.UpdateEditionAsync(_userService.GetCurrentUserObject(editionId), request.name,
				request.copyrightHolder, request.collaborators);
		}
	}
}
